...
実験は `uv run expctl new` で作る。ディレクトリを手で作ると、tier 配分・地固め・
在庫の下限のゲートが一度も評価されない。`created_by` が無い記録は lint が落とす。

記録を Bash のヒアドキュメントで書いても、ターンの終わりに Stop hook が
全件を検査する。書き方によらず規範は掛かる。
